...
// 1. TANGKAP JENIS & PERIODE
$jenis = isset($_GET['jenis']) ? $_GET['jenis'] : 'masuk'; // Default masuk
$bulan = isset($_GET['bulan']) ? $_GET['bulan'] : date('m');
$tahun = isset($_GET['tahun']) ? $_GET['tahun'] : date('Y');

// 2. ATUR JUDUL & QUERY BERDASARKAN JENIS
if ($jenis == 'masuk') {
    $judul = "REKAP TOTAL BARANG MASUK";
    $tabel = "tabel_barang_masuk";
    $kolom_jumlah = "jumlah_masuk";
    $kolom_tanggal = "tanggal_masuk";
} else {
    $judul = "REKAP TOTAL BARANG KELUAR";
    $tabel = "tabel_barang_keluar";
    $kolom_jumlah = "jumlah_keluar";
    $kolom_tanggal = "tanggal_keluar";
}

// Query Total Per Barang
$query = "SELECT b.nama_barang, s.nama_satuan, SUM(t.$kolom_jumlah) as total
          FROM $tabel t
          JOIN tabel_barang b ON t.id_barang = b.id_barang
          JOIN tabel_satuan_unit s ON b.id_satuan = s.id_satuan
          WHERE MONTH(t.$kolom_tanggal)='$bulan' AND YEAR(t.$kolom_tanggal)='$tahun'
