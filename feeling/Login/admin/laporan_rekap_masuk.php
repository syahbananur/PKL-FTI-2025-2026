...
// (B) TENTUKAN BULAN & TAHUN (Menggunakan GET agar URL bisa dicopy)
...
// (C) SIAPKAN DATA LAPORAN (QUERY GROUP BY)
$laporan_data = [];
$query_rekap = "SELECT
                    b.nama_barang,
                    s.nama_satuan,
                    SUM(bm.jumlah_masuk) AS total_bulanan
                FROM
                    tabel_barang_masuk AS bm
                JOIN
                    tabel_barang AS b ON bm.id_barang = b.id_barang
                JOIN
                    tabel_satuan_unit AS s ON b.id_satuan = s.id_satuan
                WHERE
                    MONTH(bm.tanggal_masuk) = ? AND YEAR(bm.tanggal_masuk) = ?
                GROUP BY
                    bm.id_barang, b.nama_barang, s.nama_satuan
                ORDER BY
...
<head>
    ...
    <title>Rekap Barang Masuk</title>
    ...
    <style>
        ...
        .btn-danger { background: #d9534f; } /* Merah */
        .btn-primary { background: #007bff; } /* Biru */
    ...
    <div class="dashboard-container">
        ...
        <main>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; margin-top: 30px;">
                <h3 style="margin: 0;">Total Masuk: <?php echo $nama_bulan[$bulan_pilih] . ' ' . $tahun_pilih; ?></h3>
                
                <a href="cetak_rekap.php?jenis=masuk&bulan=<?= $bulan_pilih; ?>&tahun=<?= $tahun_pilih; ?>" target="_blank" class="btn btn-secondary">
                    🖨️ Cetak Rekap
                </a>
            </div>
                ...
                <tbody>
                    ...
                    if (!empty($laporan_data)):
                        foreach ($laporan_data as $item): 
                            // Format Angka Pintar (Bulat/Koma)
                    ...
                    <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="4" class="text-center" style="padding: 20px;">Tidak ada data barang masuk untuk periode ini.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </main>
